...
class Queue extends Model
{
     ...
     */
    public $table = 'demo_workspace_queues';
    public $incrementing = false;

    public $belongsTo = [
        'application' => [\Demo\Core\Models\EngineApplication::class, 'key' => 'engine_application_id'],
        'routing_rule' => [\Demo\Workspace\Models\QueueRoutingRule::class, 'key' => 'routing_rule_id'],
    ];

    public $belongsToMany = [
        'assignment_groups' => [
            UserGroup::class,
            'table' => 'demo_workspace_queue_assignment_groups',
            'key' => 'queue_id',
            'otherKey' => 'group_id'
        ]
    ];
    ...
    {
        $queues = DB::table('demo_workspace_queues')->select(['demo_workspace_queues.id', 'demo_workspace_queues.name'])
            ->join('demo_workspace_queue_assignment_groups', 'demo_workspace_queue_assignment_groups.queue_id', '=', 'demo_workspace_queues.id')
            ->join('backend_user_groups', 'backend_user_groups.id', '=', 'demo_workspace_queue_assignment_groups.group_id')
            ...
            ->where('demo_workspace_queues.active', '=', true)
            ->orderBy('demo_workspace_queues.name', 'ASC')->get();
        $items = DB::table('demo_workspace_tasks')->select('queue_id', DB::raw('count(*) as total'))
    ...
    public function popItem($item = null)
    {
        /*if (!empty($item) && !empty($item->poped_at)) {
            ...
            $query = DB::table('demo_workspace_tasks')->select('demo_workspace_tasks.*');
        ...
        if (
            ...
            $task = DB::table('demo_workspace_tasks')->where('id', '=', $item->id)->lockForUpdate()->first();
            if (!empty($task) && !empty($task->model) && !empty($task->record_id)) {
                DB::table('demo_workspace_tasks')->where('id', '=', $item->id)->update(['poped_at' => new \DateTime()]);
