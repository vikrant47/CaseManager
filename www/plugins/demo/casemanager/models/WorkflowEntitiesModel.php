...
use Demo\Casemanager\Controllers\WorkflowTransitions;
use Model;
...
class WorkflowEntitiesModel extends Model
{
    ...
    public function makeTransition($model)
    {
        ...
        $next_queue = $this->workflow->getNextQueue($this->current_state);
        if ($next_queue === null) {
    ...
    {
        WorkflowEntitiesModel::where('id', $this->id)->update($this->toArray());
    }
}
