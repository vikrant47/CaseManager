...
class QueueModel extends Model
{
    ...
    public $belongsToMany = [
        'assignment_groups' => [
            UserGroup::class,
            'table' => 'demo_core_queue_assignment_groups',
            'key' => 'queue_id',
            'otherKey' => 'group_id'
        ]
    ];
    ...
    {
        return DB::select('SELECT queue.* from demo_core_queues queue'
            . ' join demo_core_queue_assignment_groups ag '
    ...
    public function popItem()
    {
        ...
        if (
            ...
            $queueItem = DB::table('demo_core_queue_items')->where('id', '=', $elem->id)->lockForUpdate()->get()->first();
            if (!empty($queueItem) && !empty($queueItem->item_type) && !empty($queueItem->item_id)) {
                DB::table('demo_core_queue_items')->where('id', '=', $elem->id)->delete();
